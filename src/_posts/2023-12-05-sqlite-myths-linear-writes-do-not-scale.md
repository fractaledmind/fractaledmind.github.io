...
Another way to put this in perspective is to consider how many "daily active users" (<abbr>DAU</abbr>) you can reasonably expect to handle at this throughput. Given that we could handle over 35 million requests per day, this means that we could handle **1 million <abbr title="daily active users">DAUs</abbr>** if each user made _35 write requests per day_. This is a pretty generous performance ceiling for most applications.
